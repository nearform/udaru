...
lab.experiment('test config env variable', () => {
  lab.test('set env and check config uses it', (done) => {
    ...
    const jsCode = 'node -e \'const config = require("./config")(); console.log(config.get("pgdb.password"))\''
  ...
  lab.test('set resources in env and check Udaru uses it', (done) => {
    ...
    const jsCode = 'node -e \'const config = require("./config")().get("AuthConfig"); console.log(config.resources.organizations({organizationId: 1}))\''
