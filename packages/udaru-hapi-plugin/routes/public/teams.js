  ...
  register (server, options) {
    ...
    server.route({
      ...
      async handler (request) {
        ...
        return {page, limit, ...(await request.udaruCore.teams.list({organizationId, limit: limit, page: page}))}
      },
      config: {
    ...
    server.route({
      ...
      async handler (request, h) {
        ...
        return h.response(await request.udaruCore.teams.deletePolicy({teamId, policyId, organizationId})).code(204)
      },
      config: {
        validate: {
