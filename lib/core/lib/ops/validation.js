...
const PolicyIdString = Joi.string().required().description('PolicyIdString')
// it would be ideal to put the policyid object first, however it causes a swagger doc error
// as hapi-swagger cannot interpret alternatives
const requiredPolicy = Joi.alternatives().try([
  PolicyIdString,
  PolicyIdObject
]).description('Required Policy (can be object or string)').label('RequiredPolicy')
